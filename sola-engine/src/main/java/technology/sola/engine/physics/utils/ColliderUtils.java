...
public class ColliderUtils {
   ...
   * initial collider using the sprite's dimensions. If the initial collider is a {@link ColliderShapeCircle} then
   * additional offset will be added to the collider if the sprite's dimensions are not equal.
   *
   * @param entity      the {@link Entity}
   * @param spriteSheet the {@link SpriteSheet}
   */
  public static void autoSizeColliderToSprite(Entity entity, SpriteSheet spriteSheet) {
    ...
    var sprite = spriteSheet.getSprite(spriteComponent.getSpriteId());
